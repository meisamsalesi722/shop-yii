                        ...
                        <div class="row g-3">
                            <div class="col-md-3">
                                <select class="form-select" id="status-filter" onchange="filterComments()">
                                    ...
                                    <option value="<?= Comment::STATUS_PENDING ?>">در انتظار تایید</option>
                                    <option value="<?= Comment::STATUS_APPROVED ?>">تایید شده</option>
                                    <option value="<?= Comment::STATUS_REJECTED ?>">رد شده</option>
                                    <option value="<?= Comment::STATUS_DELETED ?>">حذف شده</option>
                                </select>
                            </div>
                            <div class="col-md-3">
                                        ...
                                        <tr id="comment-row-<?= $comment->id ?>" data-status="<?= $comment->status ?>">
                                            ...
                                            <td>
                                                <div class="btn-group-vertical gap-1" style="min-width: 100px;">
                                                    <?php if ($comment->status == Comment::STATUS_PENDING || $comment->status == Comment::STATUS_REJECTED): ?>
                                                        <button class="btn btn-success btn-sm" onclick="approveComment(<?= $comment->id ?>)">
                                                            <i class="fas fa-check"></i> تایید
                                                        </button>
                                                        <?php endif; ?>
                                                        <?php if ($comment->status == Comment::STATUS_PENDING || $comment->status == Comment::STATUS_APPROVED): ?>
                                                    ...
                                                    <?php if ($comment->parent_id == null && $comment->status == Comment::STATUS_APPROVED): ?>
                                                        <a class="btn btn-info btn-sm" href="<?= Url::to( ['/admin/comment/answer' , 'id' => $comment->id]) ?>">
                                                            <i class="fas fa-trash"></i> پاسخ به کامنت
                                                        </a>
                                                    <?php endif; ?>
...
function approveComment(id) {
    ...
    let url = '/admin/comment/approve?id=' + id;
...
function rejectComment(id) {
    ...
    let url = '/admin/comment/reject?id=' + id;
    $.ajax({
        url: url,
        type: 'POST',
        dataType: 'json',
...
function deleteComment(id) {
    ...
    let url = '/admin/comment/delete?id=' + id;
    $.ajax({
        url: url,
        type: 'POST',
        dataType: 'json',
