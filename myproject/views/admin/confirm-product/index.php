                ...
                <div class="card-header bg-gradient-primary text-white p-4 border-0">
                    <div class="d-flex justify-content-between align-items-center">
                        <h4 class="mb-0 fw-bold">
                            <i class="fas fa-boxes"></i> مدیریت محصولات
                        </h4>
                        <div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="filter-section mb-4">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                <input type="text" class="form-control" id="search-input" placeholder="جستجوی نام محصول..." onkeyup="searchProducts()">
                            </div>
                            <div class="col-md-3">
                            ...
                            <tbody>
                                <?php if ($products): ?>
                                    <?php foreach ($products as $index => $product): ?>
                                        <tr id="product-row-<?= $product->id ?>" data-status="<?= $product->status ?>">
                                            <td><?= $index + 1 ?></td>
                                            <td>
                                                <?php if ($product->image): ?>
                                                    <img src="<?= Yii::getAlias('@web/uploads/images/') . $product->image ?>" alt="<?= Html::encode($product->name) ?>" 
                                            ...
                                            <td>
                                                <div class="text-nowrap">
                                                    <small class="text-muted d-block">
                                                        ...
                                                        <?= Yii::$app->formatter->asDatetime($product->created_at, 'php:Y/m/d') ?>
                                                    </small>
                                                    <small class="text-muted d-block">
                                                        <i class="far fa-clock"></i> 
                                                        <?= Yii::$app->formatter->asRelativeTime($product->created_at) ?>
                                                    </small>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="btn-group-vertical gap-1" style="min-width: 100px;">
                                                    ...
                                                    <?php if ($product->status == 0 || $product->status == 1): ?>
                                                        <button class="btn btn-danger btn-sm message" value="<?= $product->id ?>">
                                                            <i class="fas fa-times"></i> رد
                                                        </button>
                                                    <?php endif; ?>
                                                    
                                                    <a class="btn btn-info btn-sm" href="<?= Url::to(['/admin/product/view', 'id' => $product->id]) ?>">
                                ...
                                <?php else: ?>
                                    <tr>
                                        <td colspan="8" class="text-center py-5">
                                            ...
                                            <p class="text-muted mb-0">هیچ محصولی یافت نشد.</p>
                                        </td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                    ...
                    <div class="row mt-4">
                        <div class="col-md-3">
                            <div class="stat-card bg-primary bg-opacity-10 p-3 rounded-3">
                                <h6 class="text-primary">کل محصولات</h6>
...
<?php
    ...
    echo '<div id="modalAddToFactor"  >
        <input id="message" type="text">
<button id="messageBtn">ارسال</button>
    </div>  ';
...
function approveProduct(id) {
    if (!confirm('آیا از تایید این محصول مطمئن هستید؟')) {
        return;
    }
    
    $.ajax({
         url: '<?= Url::to(['/admin/confirm-product/approve', 'id' => '']) ?>' + id,
         type: 'POST',
         // data: {id: id},
         dataType: 'json',
...
function rejectProduct(id , message) {
    if (!confirm('آیا از رد این محصول مطمئن هستید؟')) {
        return;
    }
    $.ajax({
        url: '<?= Url::to(['/admin/confirm-product/reject', 'id' => '']) ?>' + id,
        type: 'POST',
