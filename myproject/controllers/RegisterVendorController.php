...
class RegisterVendorController extends Controller
{
    ...
    public function actionIndex(){
        ...
        if(Yii::$app->request->isPost){
            ...
            if($model->load(Yii::$app->request->post()) ){
                if($model->validate()){
                    ...
                    $this->redirect(['/register-vendor/confirm-otp' , 'token' => $token]);
        ...
        return $this->render('/auth/signUp-vendor' , [
            'model' =>  $model,
        ]);
    }
    ...
    public function actionConfirmOtp($token){
        ...
        if(Yii::$app->request->isPost){
            if( $otp->opt_code == Yii::$app->request->post('otp')){
                ...
                $otp->used = 1;
                $otp->save(false);
                if(Yii::$app->user->can('vendor_product')){
                    return $this->redirect('/vendor/vendor-product');
                }
                    return $this->redirect('/userpanel/login-vendor');
        ...
        return $this->render('/auth/confirm-otp-vendor');
    }
